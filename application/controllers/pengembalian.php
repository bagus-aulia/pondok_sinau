...
class Pengembalian extends CI_Controller {
	...
	{
	   // echo "mataku terbang sampai ke langit";
	    $bookCode    = $this->input->post('kd_buku');
	    $trans       = $this->m_crud->getTransID($bookCode);
	    
	    if($trans->num_rows() == 0){
	        echo "";
	    }else{
	        $trans = $trans->row();
	        $transaction = $this->m_crud->data_peminjam($trans->id_anggota);
	        
	        if($transaction->num_rows() == 0){
    	        echo "";
    	    }else{
    	        $data['bio'] = $transaction->row();
    			$data['cekdata'] = $transaction->result();
    			$data['tanggal'] = date("d M Y");
    			$this->load->view('transaksi/caridata', $data);
    	    }
	    }
